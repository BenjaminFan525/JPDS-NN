...
from utils import get_flat_params_from, set_param_values_to_model
...
class TRPO(NPG):
    ...
    def search_step_size(self, step_dir, g_flat, p_dist, data, total_steps=15, decay=0.8):
        ...
        if self.ac.encoder.nodes_encoder.frozen_gnn:
            _theta_old = get_flat_params_from(self.ac.actor_param_without_gnn)
        else:
            _theta_old = get_flat_params_from(self.ac.actor_param)
