...
from data.dataset import RLDataset_ia
from data.dataloader import RLDataLoader
import yaml
from utils import dense_fit, fit
...
def validate(data_loader, model: PG, save_dir='.', render=False):
    ...
    for batch_idx, batch in enumerate(tqdm_data_loader):
        ...
        # for item in seq_enc:
        #     print(item)
        Ts = list(map(ia_util.decode, seq_enc)) # decode the arrangements

        for bidx, (T, field_matrix, car_cfg) in enumerate(zip(Ts, field_matrices, car_cfgs)):
            r_final.append(fit(field_matrix[0], 
                    np.tile(field_matrix[1], (len(car_cfg), 1, 1, 1)), 
                    np.tile(field_matrix[2], (len(car_cfg), 1, 1, 1)), 
                    car_cfg, 
                    field_matrix[3],
                    T, 
                    tS_t=False,
    ...
    while cur_epoch < epochs:
        ...
        for batch_idx, batch in enumerate(tqdm_data_loader):
            ...
            if cur_epoch >= total_time_epoch:
                total_time = False
            for T, field_matrix, car_cfg in zip(Ts, field_matrices, car_cfgs):
                ret = dense_fit(field_matrix[0], 
                            np.tile(field_matrix[1], (len(car_cfg), 1, 1, 1)), 
                            np.tile(field_matrix[2], (len(car_cfg), 1, 1, 1)), 
                            car_cfg, 
                            field_matrix[3],
...
def main(cfg, device):
    ...
    if not cfg['test']:
        ...
        train_data = RLDataset_ia(train_data_list, cfg['veh_reciprocal'], cfg.pop('cost_limits') if 'cost_limits' in cfg else None)
        valid_data = RLDataset_ia(valid_data_list, cfg['veh_reciprocal'])
    ...
    test_data = RLDataset_ia(test_data_list, cfg['veh_reciprocal'])
