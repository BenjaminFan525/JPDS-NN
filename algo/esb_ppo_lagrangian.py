...
class ESB_PPO_Lag(PG,Lagrangian):
    '''
    ...
    def compute_loss_cost_performance(self, data):
        ...
        for key in self.constraint:
            ...
            lae.update({key: (data['cost_val'][key][:, 1:] - data['cost_val'][key][:, :-1]) + \
                        self.alphas[key] * ((data['cost_val'][key][:, :-1] ) - beta * data['cost_val'][key][:, 1:])})
    ...
    def compute_loss_pi(self, data: dict):
        ...
        for key in self.constraint:
            surrogate[key] = surrogate[key] + ((ratio_clip.detach() - 1) * self.lae[key])[val_mask[:, :-1]].mean() / (1 - self.alphas[key]) / (1 - self.gamma)
