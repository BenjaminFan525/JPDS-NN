...
def simulate(data_loader, model, model_GA, save_dir, stop_coeff=0.5, fig_interval = 10, pid = 0):
    ...
    for batch_idx, batch in enumerate(tqdm_data_loader):
        ...
        if field.num_fields == 1:
            field_list = 'all'
        elif field.num_fields <= 3:
            field_list = random.sample(range(field.num_fields), field.num_fields-1)
        else:
            field_list = random.sample(range(field.num_fields), field.num_fields-2)                
...
if __name__ == '__main__':
    ...
    save_dir = os.path.join(args.save_dir, now+'_field_ga')
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    ...
    def task(idx):
        ...
        stc = simulate(dataloaders[idx], ac, GA, save_dir, stop_coeff=args.stop_coeff, 
                       fig_interval=args.fig_interval, pid=idx)
